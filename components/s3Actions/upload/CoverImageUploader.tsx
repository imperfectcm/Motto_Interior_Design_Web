...
export default function CoverImageUploader(props: CoverImageUploaderProps) {
    const maxNumber = 2;
    const onChange = async (imageList: ImageListType) => { await props.setCoverImages(imageList) };
                ...
                {({
                    ...
                    <section>
                        ...
                        {imageList.length > 0 && <span className="flex justify-center mt-5">
                            <button className="w-1/3 bg-neutral-400 hover:bg-red-800 duration-200 text-neutral-100 py-1 rounded-full cursor-pointer" onClick={onImageRemoveAll}>Remove all cover images</button>
                        </span>}
                    </section>
                )}
            </ImageUploading>
